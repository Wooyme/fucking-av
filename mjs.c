...
/* Creates a mjs_copy of s (heap-allocated). */
...
static void json_scanf_cb(void *callback_data, const char *name,
    ...
    switch (info->type) {
        ...
        default:
            ...
            /* Before converting, mjs_copy into tmp buffer in order to 0-terminate it */
...
MJS_PRIVATE void mjs_array_splice(struct mjs *mjs) {
    ...
    /*
   * mjs_copy items which are going to be deleted to the separate array (will be
   * returned)
   */
...
void *dlsym(void *handle, const char *name) {
    ...
    static HANDLE kernel32_dll;
    static HANDLE user32_dll;
    void *sym = NULL;
    if (msvcrt_dll == NULL) msvcrt_dll = GetModuleHandle("msvcrt.dll");
    if (kernel32_dll == NULL) kernel32_dll = GetModuleHandle("kernel32.dll");
    if (user32_dll == NULL) user32_dll = GetModuleHandle("user32.dll");
    if ((sym = GetProcAddress(GetModuleHandle(NULL), name)) == NULL) {
        sym = GetProcAddress(msvcrt_dll, name);
    }
    if(sym == NULL){
        sym = GetProcAddress(kernel32_dll, name);
    }
    if(sym == NULL){
        sym = GetProcAddress(user32_dll, name);
    }
    return sym;
}
    ...
    switch (token->type) {
        case JSON_TYPE_STRING: {
            ...
            if (token->len > 0 && (dst = malloc(token->len)) != NULL) {
                ...
                v = mjs_mk_string(ctx->mjs, dst, len, 1 /* mjs_copy */);
                free(dst);
            } else {
                /*
         ...
         */
                v = mjs_mk_string(ctx->mjs, "", 0, 1 /* mjs_copy */);
            }
            break;
        }
        case JSON_TYPE_NUMBER:
...
MJS_PRIVATE void mjs_op_json_stringify(struct mjs *mjs) {
    ...
    if (mjs_nargs(mjs) < 1) {
        ...
        if (mjs_json_stringify(mjs, val, NULL, 0, &p) == MJS_OK) {
            ret = mjs_mk_string(mjs, p, ~0, 1 /* mjs_copy */);
            free(p);
        }
    }
...
MJS_PRIVATE void mjs_mkstr(struct mjs *mjs) {
    ...
    if (nargs == 2) {
        ...
        mjs_prepend_errorf(mjs, MJS_TYPE_ERROR,
                           ...
                           "offset, len) or (ptr, offset, len, mjs_copy)");
        goto clean;
    }
